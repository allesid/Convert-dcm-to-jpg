...
def convert2D_dcm2jpg(dcm_folder_path="DCM_imgs_2D", jpg_folder_path="JPG_imgs_2D",
                      img_info_fn="Images2D_information.csv", annotation_file="CrowdsCureCancer2017Annotations.csv"):
    ...
    for image in images_path:
        if image.endswith(".dcm"):
            ...
            ttt = 2000
            vfun = np.vectorize(lambda x: x if x <= ttt else ttt)
            img = vfun(img)
            # print(img[250, 250])
            img = (img / img.max()) * 255
        ...
        # information about images saved in file Images2D_information.csv
            params.to_csv(os.path.join(jpg_folder_path, ds.PatientID + "-" + img_info_fn))
    
# ="DCM_imgs" ="JPG_imgs"


def take_dcm_files(dcm_folder_path, jpg_folder_path, annots_fn, conv_fun=convert2D_dcm2jpg):
    """
...
if __name__ == '__main__':
    ...
    parser.add_argument('-a', '--annotsfile', default="CrowdsCureCancer2017Annotations.csv",
                        help='Specify name for annotation file. Default: "CrowdsCureCancer2017Annotations.csv"')
    args = parser.parse_args()
    dcm_folder_path = args.dcmpath
    jpg_folder_path = args.jpgpath
    img_info_fn = args.info
    annots_fn = args.annotsfile
    take_dcm_files(dcm_folder_path, jpg_folder_path, annots_fn, conv_fun=convert2D_dcm2jpg)
